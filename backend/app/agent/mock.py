...
---
*Report generated by Nexus AI Agent, mock mode*
"""
...
class MockLLMClient:
    ...
    async def create_message(self, prompt: str) -> str:
        """Return mock analysis JSON. ~20% high relevance, ~30% medium, ~50% noise."""
        rng = random.Random(hash(prompt) + int(time.time()) % 1000)

        roll = rng.random()
        if roll < 0.20:
            # High relevance: breaking or important
            relevance = round(rng.uniform(7.5, 9.5), 1)
            urgency = rng.choice(["breaking", "important"])
            sentiment = rng.choice(["bullish", "bearish"])
            confidence = round(rng.uniform(0.75, 0.95), 2)
        elif roll < 0.50:
            # Medium relevance
            relevance = round(rng.uniform(4.0, 7.0), 1)
            urgency = rng.choice(["important", "normal"])
            sentiment = rng.choice(["bullish", "bearish", "neutral"])
            confidence = round(rng.uniform(0.55, 0.80), 2)
        else:
            # Low relevance: noise
            relevance = round(rng.uniform(0.5, 3.5), 1)
            urgency = "noise"
            sentiment = "neutral"
            confidence = round(rng.uniform(0.30, 0.60), 2)

        assets_pool = ["BTC", "ETH", "SOL", "XRP", "ADA", "LINK", "AVAX", "DOT",
                       "SPY", "QQQ", "AAPL", "NVDA", "TSLA", "GLD", "OIL"]
        affected = rng.sample(assets_pool, rng.randint(1, 3))

        summaries = [
            f"Market impact: {sentiment} signal for {', '.join(affected)} with {urgency} urgency.",
            f"This news is {urgency} for {', '.join(affected)}. Sentiment is {sentiment}.",
            f"Analysis indicates {sentiment} outlook for {', '.join(affected)}. Relevance: {relevance}/10.",
        ]
